...
public class CompressorBlock extends BlockWithEntity {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            if (world.getBlockEntity(pos) instanceof CompressorBlockEntity compressorBlockEntity) {
                ItemScatterer.spawn(world, pos, compressorBlockEntity);
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.COMPRESSOR_BE, (world1, pos, state1, blockEntity) -> blockEntity.tick(world1, pos, state1));
    }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (world.getBlockEntity(pos) instanceof CompressorBlockEntity compressorBlockEntity && !compressorBlockEntity.getStack(1).isEmpty()) {
            world.addParticle(new ItemStackParticleEffect(ParticleTypes.ITEM, compressorBlockEntity.getStack(1)),
                    xPos + xOffsets, yPos + yOffset, zPos + zOffset, 0.0, 0.0, 0.0);
            if (!compressorBlockEntity.getStack(2).isEmpty()) {
                world.addParticle(new ItemStackParticleEffect(ParticleTypes.ITEM, compressorBlockEntity.getStack(2)),
                        xPos + xOffsets, yPos + yOffset, zPos + zOffset, 0.0, 0.0, 0.0);
            }
        }
    }
